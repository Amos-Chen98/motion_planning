...
CONFIG_FILE="${GCOPTER_SYNC_CONFIG_FILE:-$SCRIPT_DIR/sync_data.local.env}"
LOCAL_DATA_DIR="$PACKAGE_DIR/data"
LOCAL_BACKUP_DIR="$HOME/.local/share/gcopter_sync_backup/data_local"
...
REMOTE_DATA_DIR="${GCOPTER_SYNC_REMOTE_DATA_DIR:-}"
REMOTE_BACKUP_DIR="${GCOPTER_SYNC_REMOTE_BACKUP_DIR:-}"
...
load_sync_config() {
  ...
  REMOTE_DATA_DIR="${GCOPTER_SYNC_REMOTE_DATA_DIR:-${REMOTE_DATA_DIR:-}}"
  REMOTE_BACKUP_DIR="${GCOPTER_SYNC_REMOTE_BACKUP_DIR:-${REMOTE_BACKUP_DIR:-}}"

  [[ -n "$REMOTE_DATA_DIR" ]] || die "Remote data dir is not configured. Set GCOPTER_SYNC_REMOTE_DATA_DIR in $CONFIG_FILE or the environment."
  [[ -n "$REMOTE_BACKUP_DIR" ]] || die "Remote backup dir is not configured. Set GCOPTER_SYNC_REMOTE_BACKUP_DIR in $CONFIG_FILE or the environment."
}
...
create_access_sentinels() {
  ...
  sentinel_text="gcopter data sync access sentinel"
