...
class RootCircularTrajectoryPublisher:
    NODE_NAME = "root_circular_trajectory"
    ...
    def __init__(self):
        ...
        self.circle_radius = float(rospy.get_param("~circle_radius", self.DEFAULT_CIRCLE_RADIUS))
        self.circle_period = float(rospy.get_param("~circle_period", self.DEFAULT_CIRCLE_PERIOD))
        ...
        if not math.isfinite(self.circle_radius) or self.circle_radius <= 0.0:
            raise ValueError("~circle_radius must be a finite value greater than 0.")
        if not math.isfinite(self.circle_period) or self.circle_period <= 0.0:
            raise ValueError("~circle_period must be a finite value greater than 0.")
        ...
        self.center_y = None
        self.center_z = None
        self.angular_speed = 2.0 * math.pi / self.circle_period
        ...
        rospy.loginfo(
            "Waiting for start pose on %s before publishing circular root-link FLU trajectory on %s at %.1f Hz with radius %.3f m and period %.3f s; blue trajectory marker on %s.",
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.run_duration_sec > 0.0 and elapsed >= self.run_duration_sec:
                rospy.loginfo(
                    "Circular root-link trajectory reached run duration %.3f s; shutting down.",
    ...
    def build_pose_message(self, elapsed):
        theta = -math.pi / 2.0 - self.angular_speed * elapsed
        pose_msg = PoseStamped()
        pose_msg.header.stamp = rospy.Time.now()
        pose_msg.header.frame_id = self.frame_id
        pose_msg.pose.position.x = self.center_x + self.circle_radius * math.cos(theta)
        pose_msg.pose.position.y = self.center_y + self.circle_radius * math.sin(theta)
        pose_msg.pose.position.z = self.center_z

        velocity_x = self.circle_radius * self.angular_speed * math.sin(theta)
        velocity_y = -self.circle_radius * self.angular_speed * math.cos(theta)
    ...
    def start_pose_callback(self, msg):
        ...
        self.center_y = start_y + self.circle_radius
        self.center_z = start_z
        self.start_time = rospy.Time.now()
        self.has_published_initial_pose = False
        self.marker_points = [self.build_point(start_x, start_y, start_z)]
        rospy.loginfo(
            "Initialized circular root-link trajectory from %s at [%.3f, %.3f, %.3f] with center [%.3f, %.3f, %.3f]",
            rospy.resolve_name(self.start_pose_topic),
            self.start_x,
            self.start_y,
            self.start_z,
...
if __name__ == "__main__":
    try:
        RootCircularTrajectoryPublisher().run()
