...
class BatchDataAnalyzer:
    ...
    def extract_data_from_json(self, data: Dict[str, Any], filename: str) -> Dict[str, Any]:
        ...
        # Extract path length metrics if available
        root_path_length = session_info.get('root_path_length', '')
        generalized_path_length = session_info.get('generalized_path_length', '')
        
        # If mission failed, set computation time and path lengths to '-'
        if not mission_success:
            computation_time = '-'
            root_path_length = '-'
            generalized_path_length = '-'
    ...
    def print_summary(self, data: List[Dict[str, Any]]) -> None:
        ...
        success_counts = {}
        algorithm_times = {}
        algorithm_root_lengths = {}
        algorithm_generalized_lengths = {}
        total_successful = 0
        ...
        for record in data:
            ...
            if mission_success:
                ...
                # Collect root path lengths for successful cases
                if (record.get('root_path_length', '') != '' and 
                    record.get('root_path_length', '') != '-' and
                    isinstance(record.get('root_path_length', ''), (int, float))):
                    
                    if algorithm not in algorithm_root_lengths:
                        algorithm_root_lengths[algorithm] = []
                    algorithm_root_lengths[algorithm].append(float(record['root_path_length']))
                
                # Collect generalized path lengths for successful cases
                if (record.get('generalized_path_length', '') != '' and 
                    record.get('generalized_path_length', '') != '-' and
                    isinstance(record.get('generalized_path_length', ''), (int, float))):
                    
                    if algorithm not in algorithm_generalized_lengths:
                        algorithm_generalized_lengths[algorithm] = []
                    algorithm_generalized_lengths[algorithm].append(float(record['generalized_path_length']))
        ...
        for algorithm in sorted(algorithm_counts.keys()):
            ...
            # Print root path length statistics for successful cases
            if algorithm in algorithm_root_lengths and algorithm_root_lengths[algorithm]:
                lengths = algorithm_root_lengths[algorithm]
                mean_length = sum(lengths) / len(lengths)
                variance = sum((l - mean_length) ** 2 for l in lengths) / len(lengths)
                std_dev = variance ** 0.5
                
                print(f"  Mean root path length: {mean_length:.3f}")
                print(f"  Std dev root path length: {std_dev:.6f}")
            else:
                print(f"  Mean root path length: N/A (no data)")
            
            # Print generalized path length statistics for successful cases
            if algorithm in algorithm_generalized_lengths and algorithm_generalized_lengths[algorithm]:
                lengths = algorithm_generalized_lengths[algorithm]
                mean_length = sum(lengths) / len(lengths)
                variance = sum((l - mean_length) ** 2 for l in lengths) / len(lengths)
                std_dev = variance ** 0.5
                
                print(f"  Mean generalized path length: {mean_length:.3f}")
                print(f"  Std dev generalized path length: {std_dev:.6f}")
            else:
                print(f"  Mean generalized path length: N/A (no data)")
...
def main():
    """Main function."""
    analyzer = BatchDataAnalyzer("20251221_proposed_batch_200")
