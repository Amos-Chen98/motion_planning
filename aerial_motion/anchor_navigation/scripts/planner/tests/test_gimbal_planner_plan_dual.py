...
import numpy as np
from gimbal_yaw_planner_bspline_dual import GimbalYawPlanner
import time
...
def test_gimbal_planner_plan_function():
    ...
    # Test the plan function without initial hint
    ...
    try:
        ...
        # Extract results from the new return format (tuple with 3 elements)
        if plan_result is not None and isinstance(plan_result, tuple) and len(plan_result) == 3:
            des_gimbal_angles_array, opt_init_gimbal_angles, objective_history = plan_result
        else:
            des_gimbal_angles_array = None
            opt_init_gimbal_angles = None
            objective_history = None

        # Check if the result is valid
        if (des_gimbal_angles_array is not None and 
            opt_init_gimbal_angles is not None and 
            not np.any(np.isnan(des_gimbal_angles_array)) and 
            not np.any(np.isnan(opt_init_gimbal_angles))):
            print(f"✓ Plan function succeeded!")
            print(f"Execution time: {execution_time:.4f} seconds")
            print(f"Optimized initial gimbal angles: {np.round(np.degrees(opt_init_gimbal_angles)).astype(int)}°")
    ...
    return joint_angles_array, des_gimbal_angles_array, opt_init_gimbal_angles, objective_history, time_array, config_descriptions, success, error_msg, gimbal_yaw_planner, execution_time


def plot_plan_results(joint_angles_array, des_gimbal_angles_array, opt_init_gimbal_angles, objective_history, time_array, config_descriptions, success, gimbal_yaw_planner):
    """
    ...
    Args:
        ...
        opt_init_gimbal_angles: Optimized initial gimbal angles (4,) or None
    ...
    title_suffix = f' (Init: {np.round(np.degrees(opt_init_gimbal_angles)).astype(int)}°)' if opt_init_gimbal_angles is not None else ''
    ax2.set_title(f'Output Gimbal Angle Trajectory (B-spline Optimized){title_suffix}', fontsize=14, fontweight='bold')
...
def print_plan_summary_statistics(joint_angles_array, des_gimbal_angles_array, opt_init_gimbal_angles, objective_history, time_array, success, error_msg, execution_time):
    """
    ...
    if success and des_gimbal_angles_array is not None:
        ...
        if opt_init_gimbal_angles is not None:
            print(f"Optimized initial gimbal angles: {np.round(np.degrees(opt_init_gimbal_angles)).astype(int)}°")
        
        if objective_history is not None:
            print(f"Optimization iterations: {len(objective_history)}")
...
def run_multiple_test_scenarios():
    ...
    for i, scenario in enumerate(scenarios):
        ...
        try:
            ...
            # Test plan function without initial hint
            plan_result = gimbal_yaw_planner.plan(
                des_joint_angles=joint_angles_array,
            ...
            if (des_gimbal_angles_array is not None and 
                ...
                final_objective = objective_history[-1] if objective_history is not None else None
                opt_init_angles_deg = np.round(np.degrees(opt_init_gimbal_angles)).astype(int)
                ...
                results.append({
                    ...
                    "opt_init_angles": opt_init_angles_deg
                })
            else:
    ...
    for result in results:
        ...
        if result["success"]:
            ...
            if result.get('final_objective') is not None:
                print(f"    Final objective: {result['final_objective']:.6f}")
            if result.get('opt_init_angles') is not None:
                print(f"    Optimized initial angles: {result['opt_init_angles']}°")
        else:
...
if __name__ == "__main__":
    try:
        # Run the main test
        (joint_angles_array, des_gimbal_angles_array, opt_init_gimbal_angles, objective_history, 
         time_array, config_descriptions, success, error_msg, gimbal_yaw_planner, execution_time) = test_gimbal_planner_plan_function()

        # Print summary statistics
        print_plan_summary_statistics(joint_angles_array, des_gimbal_angles_array, opt_init_gimbal_angles, 
                                    objective_history, time_array, success, error_msg, execution_time)

        # Create plots if successful
        if success:
            plot_plan_results(joint_angles_array, des_gimbal_angles_array, opt_init_gimbal_angles, objective_history,
