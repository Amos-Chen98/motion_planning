...
class GimbalYawPlanner(ClampedBSpline):
    def __init__(self, robot_urdf, planner_config):
        ...
        self.opt_variable_num = self.control_points_num * self.gimbal_num + self.gimbal_num
        self.sample_hz = 10
        self.output_hz = 60
    ...
    def plan(self, des_joint_angles: np.ndarray, time: float, init_gimbal_angles: np.ndarray) -> tuple:
        """
        Inputs:
            ...
            init_gimbal_angles: np.ndarray of (gimbal_num,), initial gimbal angles
        Outputs:
        ...
        self.init_gimbal_angles = init_gimbal_angles
        ...
        final_gimbal_angles, opt_control_points = self._run_optimization(opt_variables)

        # Build B-spline and generate trajectory
        if opt_control_points is not None:
            des_gimbal_angles_array = self.get_des_gimbal_angles_array(init_gimbal_angles, final_gimbal_angles, opt_control_points)
            # Return both the trajectory and the objective history as a tuple
            return des_gimbal_angles_array, np.array(self.objective_history)
        else:
            return None, None
    ...
    def _init_opt_variables(self):
        ...
        opt_variables = np.array([-4.041231, 0.219394, 1.713213, 0.602019, 0.467707, -3.866992, 0.207325,
                                  -6.027583, -5.336373, 1.368932, -3.615158, 2.865992, -6.241531, -4.611550,
                                  0.504434, 1.351902, -4.924941, 1.411619, -5.728968, 1.885829, 0.221075,
                                  1.416267, -3.628804, -4.662146])
    ...
    def _run_optimization(self, opt_variables):
        try:
            ...
            if opt_result < 0:
                ...
                final_gimbal_angles, control_points = self._retrieve_x(x_opt)

                return final_gimbal_angles, control_points
    ...
    def _objective(self, x: np.ndarray, grad: np.ndarray):
        ...
        """
        final_gimbal_angles, control_points = self._retrieve_x(x)

        self.build_full_spline(
            init_gimbal_angles=self.init_gimbal_angles,
    ...
    def _retrieve_x(self, x: np.ndarray):
        final_gimbal_angles = x[:self.gimbal_num]
        x_intermediate_control_points = x[self.gimbal_num:]
